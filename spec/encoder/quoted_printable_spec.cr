...
struct QuotedPrintableEncoderTest < ASPEC::TestCase
  def test_quoted_printable_encode : Nil
    encoder = AMIME::Encoder::QuotedPrintable.new

    encoder.quoted_printable_encode("test").should eq "test"
    encoder.quoted_printable_encode("this is a foo").should eq "this is a foo"

    encoder.quoted_printable_encode("This is a sample string with special characters: ä, ö, ü, and ß.").should eq <<-TXT
      This is a sample string with special characters: =C3=A4, =C3=B6, =C3=BC, an=\r
      d =C3=9F.
      TXT

    encoder.quoted_printable_encode("Iñtërnâtiônàlizætiøn☃💩").should eq <<-TXT
  ...
  def test_quoted_printable_encode_permitted_characters_are_not_encoded : Nil
    encoder = Athena::MIME::Encoder::QuotedPrintable.new

    ((33..60).to_a + (62..126).to_a).each do |ord|
      char = ord.chr.to_s
      encoder.quoted_printable_encode(char).should eq char
    end
  end
  ...
  def test_quoted_printable_encode_crlf_is_left_alone : Nil
    ...
    string = "a\r\nb\r\nc\r\n"
    encoder.quoted_printable_encode(string).should eq string
  end
  ...
  def test_quoted_printable_encode_always_encodes_tabs : Nil
    ...
    string = "a\t\t\r\nb"
    encoder.quoted_printable_encode(string).should eq "a=09=09\r\nb"
  end
  ...
  def test_quoted_printable_encode_encodes_space_before_newline : Nil
    ...
    string = "a  \r\nb"
    encoder.quoted_printable_encode(string).should eq "a =20\r\nb"
  end

  def test_lines_longer_than_76_characters_are_soft_broken : Nil
    encoder = Athena::MIME::Encoder::QuotedPrintable.new

    encoder.quoted_printable_encode("a" * 140).should eq <<-TXT
      aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa=\r\naaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa
      TXT
  end

  def test_bytes_below_permitted_range_are_encoded : Nil
    encoder = Athena::MIME::Encoder::QuotedPrintable.new

    (0..31).each do |byte|
      char = byte.chr.to_s

      encoder.quoted_printable_encode(char).should eq sprintf("=%02X", byte)
    end

    # Allows spaces
    encoder.quoted_printable_encode(" ").should eq " "
  end
end
