...
struct QuotedPrintableEncoderTest < ASPEC::TestCase
  def test_quoted_printable_encode : Nil
    AMIME::Encoder::QuotedPrintable.quoted_printable_encode("test").should eq "test"
    AMIME::Encoder::QuotedPrintable.quoted_printable_encode("this is a foo").should eq "this is a foo"

    AMIME::Encoder::QuotedPrintable.quoted_printable_encode("This is a sample string with special characters: ä, ö, ü, and ß.").should eq <<-TXT
      This is a sample string with special characters: =C3=A4, =C3=B6, =C3=BC, an=\r
      d =C3=9F.
      TXT

    AMIME::Encoder::QuotedPrintable.quoted_printable_encode("Iñtërnâtiônàlizætiøn☃💩").should eq <<-TXT
  ...
  def test_quoted_printable_encode_permitted_characters_are_not_encoded : Nil
    ((33..60).to_a + (62..126).to_a).each do |ord|
      char = ord.chr.to_s
      AMIME::Encoder::QuotedPrintable.quoted_printable_encode(char).should eq char
    end
  end

  def test_quoted_printable_encode_crlf_is_left_alone : Nil
    string = "a\r\nb\r\nc\r\n"
    AMIME::Encoder::QuotedPrintable.quoted_printable_encode(string).should eq string
  end
  ...
  def test_quoted_printable_encode_encodes_space_before_newline : Nil
    Athena::MIME::Encoder::QuotedPrintable
      .quoted_printable_encode("a  \r\nb")
      .should eq "a =20\r\nb"
  end

  def test_quoted_printable_encode_lines_longer_than_76_characters_are_soft_broken : Nil
    Athena::MIME::Encoder::QuotedPrintable.quoted_printable_encode("a" * 140).should eq <<-TXT
  ...
  def test_quoted_printable_encode_bytes_below_permitted_range_are_encoded : Nil
    ...
    # Allows spaces
    Athena::MIME::Encoder::QuotedPrintable.quoted_printable_encode(" ").should eq " "
  end
end
